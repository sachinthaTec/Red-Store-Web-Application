...
if (isset($_SESSION["u"])) {
    ...
    if (empty($line1)) {
        echo ("Please Enter the  Line1");
    } else if (empty($line2)) {
        echo ("Please Enter the  Line2");
    }  else if (empty($city)) {
        echo ("Please Select the city");
    }  else if (empty($pc)) {
        echo ("Please Enter the Post Code");
    }  else {
    ...
    if (isset($_FILES["img"])) {
        ...
        if (in_array($file_type, $allowed_image_extentions)) {
            ...
            $image_rs = Database::search("SELECT * FROM `profile_image` WHERE `users_email` = '" . $user_email . "'");
